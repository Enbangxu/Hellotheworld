...
export function DeepSeekTutorPanel({ topicId, locale }: { topicId: string; locale: Locale }) {
  ...
  const run = async (mode: TutorMode) => {
    ...
    try {
      const response = await fetch("/api/learning/deepseek-tutor", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ locale, topicId, mode, question, userAnswer: mode === "diagnose" ? question : undefined }) });
