...
type ChatMessage = { role: "assistant" | "user"; content: string; keywords?: Keyword[]; suggestedQuestions?: string[] };
...
export function AIChat({ open: controlledOpen, onOpenChange }: { open?: boolean; onOpenChange?: (open: boolean) => void }) {
  ...
  async function submitMessage(message: string) {
    ...
    try {
      ...
      const result = await response.json() as { answer?: string; reply?: string; keywords?: Keyword[]; suggestedQuestions?: string[]; error?: { message?: string } };
      const answer = result.answer || result.reply;
      if (!response.ok || !answer) throw new Error(result.error?.message || "AI 服务暂时不可用");
      setMessages((items) => [...items, { role: "assistant", content: answer, keywords: result.keywords || [], suggestedQuestions: result.suggestedQuestions || [] }]);
    } catch (error) {
  ...
  return <>
    ...
    {open && <div className="v15-chat" role="dialog" aria-modal="true" aria-label="DeepSeek AI 聊天">
      ...
      <div className="v15-chat-messages" aria-live="polite">{messages.map((message, index) => message.role === "user" ? <p key={`user-${index}`} className="v15-message-user">{message.content}</p> : <AIResponse key={`assistant-${index}`} answer={message.content} keywords={message.keywords || []} suggestedQuestions={message.suggestedQuestions || []} onAsk={submitMessage} />)}{loading && <p className="v15-message-ai flex items-center gap-2"><LoaderCircle className="animate-spin" size={16} /> AI 正在思考并分析关键词…</p>}</div>
