...
export async function askDeepSeek(
  ...
  const context = {
    ...
    quickLesson: {
      meaning: zh(topic.quickLesson.meaning),
      plainExplanation: zh(topic.quickLesson.plainExplanation),
      microExample: {
        setup: zh(topic.quickLesson.microExample.setup),
        thinking: zh(topic.quickLesson.microExample.thinking),
        result: zh(topic.quickLesson.microExample.result),
      },
      useWhen: zh(topic.quickLesson.useWhen),
      memoryLine: zh(topic.quickLesson.memoryLine),
    },
    introduction: zh(topic.introduction),
    sections: topic.sections.map((section) => ({
      title: zh(section.title),
      paragraphs: section.paragraphs.map(zh),
      bullets: section.bullets?.map(zh),
      formula: section.formula,
    })),
    formula: topic.formula,
    methodSteps: topic.methodSteps.map(zh),
    workedExamples: topic.workedExamples.map((example) => ({
      title: zh(example.title),
      problem: zh(example.problem),
      steps: example.steps.map(zh),
      answer: zh(example.answer),
      explanation: zh(example.explanation),
    })),
    commonMistakes: topic.commonMistakes.map((mistake) => ({
      mistake: zh(mistake.mistake),
      whyWrong: zh(mistake.whyWrong),
      correction: zh(mistake.correction),
    })),
    quickCheck: {
      question: zh(topic.quickCheck.question),
      options: topic.quickCheck.options.map(zh),
      answer: zh(topic.quickCheck.answer),
      explanation: zh(topic.quickCheck.explanation),
    },
  };
  const system = [
    ...
    "simplify 模式一次只解释一个概念：answer 中文不超过120字，steps最多3步且每步不超过35字，keyInsight不超过50字；必须给一个具体例子，专业词立即用日常语言解释。",
