...
import { POST } from "@/src/app/api/learning/deepseek-tutor/route";
...
describe("DeepSeek learning tutor route", () => {
  it("returns 400 for invalid input", async () => {
    const response = await POST(new NextRequest("http://localhost/api/learning/deepseek-tutor", { method: "POST", body: JSON.stringify({ locale: "invalid" }), headers: { "content-type": "application/json" } }));
  ...
  it("returns a stable 503 without a DeepSeek key while static data remains available", async () => {
    ...
    const response = await POST(new NextRequest("http://localhost/api/learning/deepseek-tutor", { method: "POST", body: JSON.stringify({ locale: "zh", topicId: topic.id, mode: "ask" }), headers: { "content-type": "application/json", "x-forwarded-for": "route-test" } }));
