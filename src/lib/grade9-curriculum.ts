import { subjects, type LearningLocale, type Topic } from "@/src/data/grade9";
...
export const getMicroLesson = (topic: Topic, slugOrId: string) => topic.microLessons.find((micro) => micro.slug === slugOrId || micro.id === slugOrId);
export const allMicroLessons = () => allTopics().flatMap((entry) => entry.topic.microLessons.map((microLesson) => ({ ...entry, microLesson })));
...
export const localize = (value: Record<LearningLocale, string>, locale: LearningLocale) => value[locale] || value.zh;
